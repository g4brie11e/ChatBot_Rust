...
pub async fn generate_reply(
    ...
    if user_msg.trim().eq_ignore_ascii_case("reset")
        || user_msg.trim().eq_ignore_ascii_case("cancel")
    {
        return (
            "Conversation reset. Please select your language: English, Polish, French, Spanish."
                .to_string(),
    ...
    if user_msg.trim().eq_ignore_ascii_case("status") {
        let status_msg = match current_state {
            ...
            ConversationState::AskingProjectConfirmation => {
                "I am waiting for confirmation to start a project inquiry."
            }
        };
    ...
    match current_state {
        ConversationState::AskingLanguage => {
            ...
            if msg_lower.contains("english") || msg_lower == "en" {
                ...
                (
                    get_localized_msg("greeting", "en"),
                    ConversationState::Idle,
                    data,
                )
            } else if msg_lower.contains("polish")
                || msg_lower.contains("polski")
                || msg_lower == "pl"
            {
                data.language = "pl".to_string();
                (
                    get_localized_msg("greeting", "pl"),
                    ConversationState::Idle,
                    data,
                )
            } else if msg_lower.contains("french")
                || msg_lower.contains("francais")
                || msg_lower == "fr"
            {
                data.language = "fr".to_string();
                (
                    get_localized_msg("greeting", "fr"),
                    ConversationState::Idle,
                    data,
                )
            } else if msg_lower.contains("spanish")
                || msg_lower.contains("espanol")
                || msg_lower == "es"
            {
                data.language = "es".to_string();
                (
                    get_localized_msg("greeting", "es"),
                    ConversationState::Idle,
                    data,
                )
                ...
                (
                    get_localized_msg("greeting", &lang),
                    ConversationState::Idle,
                    data,
                )
            } else {
                ...
                Intent::Unknown => {
                    if let Some(ai_reply) = call_mistral(&history).await {
                        ...
                        (
                            format!(
                                "{} '{}'.",
                                get_localized_msg("unknown", &data.language),
                                user_msg
                            ),
                            ConversationState::Idle,
                            data,
        ...
        ConversationState::AskingName => {
            ...
            if is_valid_name(name) {
                ...
                (
                    format!(
                        "{}, {}! {}",
                        get_localized_msg("nice_meet", &data.language),
                        name,
                        get_localized_msg("ask_email", &data.language)
                    ),
                ...
                (
                    get_localized_msg("invalid_name", &data.language),
                    ConversationState::AskingName,
                    data,
                )
            }
        ...
        ConversationState::AskingEmail => {
            if user_msg.contains("@") {
                ...
                (
                    get_localized_msg("ask_budget", &data.language),
                    ConversationState::AskingBudget,
                    data,
                )
            } else {
                (
                    get_localized_msg("invalid_email", &data.language),
                    ConversationState::AskingEmail,
                    data,
                )
            }
        ...
        ConversationState::AskingBudget => {
            if user_msg.chars().any(|c| c.is_numeric()) {
                ...
                (
                    get_localized_msg("ask_details", &data.language),
                    ConversationState::AskingProjectDetails,
                    data,
                )
            } else {
                (
                    get_localized_msg("invalid_budget", &data.language),
                    ConversationState::AskingBudget,
                    data,
                )
            }
        ...
        ConversationState::AskingProjectConfirmation => {
            let msg_lower = user_msg.trim().to_lowercase();
            if msg_lower.contains("yes")
                || msg_lower.contains("sure")
                || msg_lower.contains("ok")
                || msg_lower.contains("yep")
                || msg_lower.contains("tak")
                || msg_lower.contains("oui")
                || msg_lower.contains("si")
            {
...
fn get_reminder(state: &ConversationState) -> &str {
    match state {
        ...
        ConversationState::AskingProjectConfirmation => {
            "Would you like to start a project inquiry? (yes/no)"
        }
        _ => "",
    }
}
...
fn infer_language(msg: &str) -> Option<String> {
    let msg_lower = msg.to_lowercase();
    // Spanish
    if msg_lower.contains("hola")
        || msg_lower.contains("ayuda")
        || msg_lower.contains("contacto")
        || msg_lower.contains("precio")
        || msg_lower.contains("servicios")
    {
    ...
    // Polish
    if msg_lower.contains("cześć")
        || msg_lower.contains("czesc")
        || msg_lower.contains("pomoc")
        || msg_lower.contains("kontakt")
        || msg_lower.contains("cena")
        || msg_lower.contains("usługi")
...
fn get_localized_msg(key: &str, lang: &str) -> String {
    match (lang, key) {
        ...
        ("pl", "pricing_start") => {
            "Nasze strony zaczynają się od 1000$. Czy chcesz rozpocząć wycenę?".to_string()
        }
        ("pl", "pricing_info") => "Nasze strony zaczynają się od 1000$.".to_string(),
        ("pl", "contact_info") => {
            "Możesz się z nami skontaktować pod adresem contact@webagency.com lub +1-555-0199."
                .to_string()
        }
        ("pl", "help_info") => {
            "Mogę pomóc z wyceną, kontaktem lub rozpoczęciem nowego projektu.".to_string()
        }
        ...
        ("pl", "ok_no_problem") => {
            "Ok, nie ma problemu. Daj znać jeśli będziesz czegoś potrzebować.".to_string()
        }
        ("pl", "help_interruption") => {
            "Aktualnie pytam o szczegóły. Wpisz 'reset' aby zacząć od nowa.".to_string()
        }

        // French
        ("fr", "greeting") => {
            "Bonjour! Bienvenue dans notre agence. Comment puis-je vous aider ?".to_string()
        }
        ("fr", "website_start") => {
            "Nous serions ravis de vous aider. Pour commencer, quel est votre nom ?".to_string()
        }
        ("fr", "pricing_start") => {
            "Nos sites commencent à 1000$. Voulez-vous lancer une demande ?".to_string()
        }
        ("fr", "pricing_info") => "Nos sites commencent à 1000$.".to_string(),
        ("fr", "contact_info") => {
            "Vous pouvez nous joindre à contact@webagency.com ou +1-555-0199.".to_string()
        }
        ("fr", "help_info") => {
            "Je peux vous aider avec les prix, le contact ou un nouveau projet.".to_string()
        }
        ("fr", "services_info") => {
            "Nous proposons du développement Web, des applications et du SEO.".to_string()
        }
        ...
        ("fr", "invalid_name") => {
            "Ce nom ne semble pas valide. Utilisez uniquement des lettres.".to_string()
        }
        ...
        ("fr", "help_interruption") => {
            "Je demande actuellement vos détails. Tapez 'reset' pour recommencer.".to_string()
        }

        // Spanish
        ("es", "greeting") => {
            "¡Hola! Bienvenido a nuestra agencia. ¿Cómo puedo ayudarte?".to_string()
        }
        ("es", "website_start") => {
            "Nos encantaría ayudar. Para empezar, ¿cuál es tu nombre?".to_string()
        }
        ("es", "pricing_start") => {
            "Nuestros sitios comienzan en $1000. ¿Quieres iniciar una consulta?".to_string()
        }
        ("es", "pricing_info") => "Nuestros sitios comienzan en $1000.".to_string(),
        ("es", "contact_info") => {
            "Puedes contactarnos en contact@webagency.com o +1-555-0199.".to_string()
        }
        ("es", "help_info") => {
            "Puedo ayudarte con precios, contacto o un nuevo proyecto.".to_string()
        }
        ...
        ("es", "ask_details") => {
            "Entendido. Finalmente, describe brevemente tu proyecto.".to_string()
        }
        ("es", "invalid_budget") => "Ingresa un monto válido (ej. 1000).".to_string(),
        ("es", "ok_no_problem") => "Bien, no hay problema.".to_string(),
        ("es", "help_interruption") => {
            "Estoy pidiendo tus detalles. Escribe 'reset' para reiniciar.".to_string()
        }

        // Default (English)
        (_, "greeting") => "Hello! Welcome to our agency. How can I help you today?".to_string(),
        (_, "website_start") => {
            "We'd love to help with your website. To start, could you tell me your name?"
                .to_string()
        }
        (_, "pricing_start") => {
        ...
        (_, "invalid_name") => {
            "That doesn't look like a valid name. Please use letters only (no numbers).".to_string()
        }
        (_, "ask_budget") => "Thanks! What is your estimated budget for this project?".to_string(),
        (_, "invalid_email") => {
            "That doesn't look like a valid email. Please try again.".to_string()
        }
        (_, "ask_details") => {
            "Got it. Finally, please describe your project requirements briefly.".to_string()
        }
        ...
        (_, "help_interruption") => {
            "I'm currently asking for your details. You can type 'reset' to start over.".to_string()
        }
...
async fn call_mistral(history: &[Message]) -> Option<String> {
    ...
    let mut messages = vec![MistralMessage {
        role: "system".to_string(),
        content: "You are a helpful assistant for a web agency. Keep answers concise.".to_string(),
    }];
