...
pub async fn chat_handler(
    ...
    let (mut reply, next_state, next_data) = generate_reply(
        current_state.clone(),
        trimmed,
        current_data.clone(),
        history,
    ...
    // Check if the flow just finished
    if current_state == ConversationState::AskingProjectDetails
        && next_state == ConversationState::Idle
    {
        // Save the lead to the json file
    ...
    state
        .sessions
        .append_message(&session_id, MessageRole::Bot, &reply)
        .await;
...
// read .json and return as JSON array
